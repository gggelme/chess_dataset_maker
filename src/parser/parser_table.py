...
def _rotation_matrix(rotacion, lado):
    """Devuelve la homografía 3x3 equivalente a la rotación de cv2."""
...
def _identificar_horiz_vert(grupo_a, grupo_b):
    """Devuelve (horiz, vert) según cuál grupo tiene theta más cercano a pi/2."""
    def _media_theta(g):
...
class ParserTable:
    ...
    def correct_perspective(self, lado: int = LADO_DESTINO):
        """Transforma el tablero detectado a un cuadrado en HSV."""
        ...
        self.tablero_hsv = self._normalizar_iluminacion_hsv(self.tablero_hsv)
    ...
    def detect_grid_lines(self):
        ...
        v       = np.median(bin_img)
        sigma   = 0.33
        canny   = cv2.Canny(bin_img, int(max(0,   (1.0 - sigma) * v)), int(min(255, (1.0 + sigma) * v)), apertureSize=3)
    ...
    def aplicar_warp(self, img):
        return cv2.warpPerspective(img, self.H, (800, 800))
...
if __name__ == "__main__":
    ...
    if img_gris is None:
        print(f"No se pudo cargar la imagen de prueba en: {ruta}")
        exit()
    ...
    # Dibujamos la grilla manualmente con OpenCV sobre el tablero rectificado
    img_board = tablero_rgb.copy()
    for y in y_pos:
        cv2.line(img_board, (0, y), (800, y), (255, 0, 0), 2)
    for x in x_pos:
        cv2.line(img_board, (x, 0), (x, 800), (255, 0, 0), 2)
    ...
    axs[2].set_title("3. Grilla detectada")
    axs[2].axis("off")
    plt.tight_layout()
    plt.show()