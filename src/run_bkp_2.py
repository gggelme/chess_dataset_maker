...
def main():
    ...
    frames_estables   = 0
    ultimo_estado     = None
    post_interrupcion = False  # True luego de una INTERRUPCION; resetea tras detección o settle
    ...
    while cap.isOpened():
        ...
        if frame_ref is None:
            ...
            try:
                ...
                live_board.actualizar(tablero.matriz)
        ...
        if (
            ...
            # Sin movimiento previo o tablero sin cambios reales: refrescar referencia
            if not post_interrupcion or energia < UMBRAL_MINIMO:
                frame_ref         = gris.copy()
                frames_estables   = 0
            ...
            top4, energias_celdas = obtener_top_celdas(frame_ref, ref_nueva, parser, UMBRAL_PIEZA)
            print(f"  Top celdas: {top4}  turno={tablero.turno}")
